...
def get_local_time(ctx, timezone):
    # Was a fixed offset off a UTC-05 style table, with the setting's own help
    # text telling people to "adjust manually when daylight saving time
    # changes". It is a real zone now and the changeover is worked out below.
    local_unix = ctx.now.unix + zone_offset_at(timezone, ctx.now.unix // 60) * 60
...
def home(c, ctx):
    ...
    timezone = str(safe_input(ctx, "timezone", "America/New_York")).strip()
